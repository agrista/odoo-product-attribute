...
class PricelistItem(models.Model):
    ...
    @api.constrains('compute_price', 'min_quantity', 'applied_on', 'base')
    ...
    def _check_tier_validations(self):
        ...
        if self.compute_price == 'tiered':
            if self.base != 'list_price':
                messages += ('The pricelist item must be based '
            ...
            if self.min_quantity < 1:
                messages += ('Must include minimum quantity of 1 or '
    ...
    @api.onchange('tiered_price', 'min_quantity', 'compute_price',
                  'product_tmpl_id', 'applied_on', 'base')
